    ...
    public boolean tilt(Side side) {
        ...
        for (int c = 0; c < board.size(); c++) {
            for (int r = board.size() -1 ; r >= 0; r--) {
                ...
                int ThePlace = r; // 初始目标位置是自己

                if (t != null) { // 只有当前方块不为空时，才尝试移动
                    if(r != board.size() -1){
                        for (int i = r+1; i < board.size(); i++) {
                            Tile targetTile = board.tile(c, i);
                            if (targetTile == null) {
                                // 如果目标位置为空，更新目标位置
                                ThePlace = i;
                            } else if (targetTile.value() == t.value() && !merged[c][i]) {
                                // 如果目标位置有相同值且未合并过，进行合并
                                ThePlace = i;
                                score += t.value() * 2; // 更新分数
                                merged[c][i] = true; // 标记此位置已合并
                                break;
                            } else {
                                // 既不是空位，也不能合并，停止检查
                                break;
